var $User = function () {
    ...
    innerText = (function () {
        ...
        LoadProgress: function (r, l, a) {
            ...
            SetHidden($("imgGQJC"));
            var p = oS, j = [], i = p.PicArr, k = p.PName, s = p.ZName, u = 0, d = document.createTextNode("Đang tải。。。"),
                t = GetX(11), g = oGd.$LF, b = oGd.$ZF, c = oS.R + 1, e = p.LoadImage, h = p.CheckImg, f = p.InitPn, m, q;
        ...
        Init: function (b) {
            ...
            (function (p) {
                ...
                while (n--) {
                    ...
                    !c.HP ? (i.splice(n, 1), d(c)) : !(c.FreeFreezeTime || c.FreeSetbodyTime) ? (c.beAttacked && !c.isAttacking && c.JudgeAttack(), g = c.id, !c.isAttacking ? (a = c.Speed, !c.WalkDirection ? ((l = c.AttackedRX -= a) < 0 ? (i.splice(n, 1), c.Die(2), d(c)) : (l < 100 && !c.PointZombie && (c.PointZombie = 1, c.ChangeR({
                        R: p,
                        ar: [oS.R - 1],
        ...
        Init: function (b) {
            ...
            var f = new RegExp(d + ",", "g"), h = (j.toString() + ",").replace(f, "┢,").replace(/[^,┢]/g, ""), i = 0, g = 0,
                b = [];
        ...
        var g = (a = a || event).clientX, e = a.clientY + document.body.scrollTop, d = h.PName.prototype, c = ArCard.length,
            b;
        oS.Chose = 1;
    ...
    }, GameOver = function (e, name) {
        innerText($("dNameWinner").textContent = "Chúc mừng người chơi " + name + " đã công thành");
        SetVisible($("dNameWinner"))
     ...
     */
    BirthZombie = function (name, zombie, delayTime, numOfLane = 1) {
        let zombieArr = []
        let zombieNode = (zombieArr[0] = new zombie).prepareBirth(delayTime, name, numOfLane);
    ...
    WordUTF8 = '<div id="dLogo" style="position:absolute;width:900px;height:600px;background:#000 url(images/Logo.jpg) no-repeat;z-index:1"><image src="images/LogoWord.jpg" style="position:absolute;left:320px;top:15px"><div id="LogoWord" style="position:absolute;color:#FF0;top:480px;width:100%;height:100px"><span style="position:absolute;width:150px;height:72px;left:180px;top:-80;cursor:pointer;background:url(images/logo.png)" onclick=window.open("http://choinhanh.vn")></span><span style="position:absolute;width:332px;height:94px;left:90px;top:5;cursor:pointer;background:url(images/interface/LoadBar.png)" onclick="SetBlock($(\'dSurface\'),$(\'iSurfaceBackground\'))"></span><span style="position:absolute;font-size:14px;left:480px;top:60px;line-height:1.5;font-weight:bold;cursor:pointer" onclick="launchFullscreen(document.documentElement)">Toàn màn hình</span></span></div></div>',
