...
const ZombieWin = function (e, name, score, userId, avatar) {
    ...
    innerText(NewEle("boxTextWin" + name, "div", "text-shadow: 1px 1px 0 black, 2px 2px 0 black;font: bold 30px Helvetica, Arial, Sans-Serif;color: #ff4e4e;padding: 80px 0px;text-align: center;", 0, NewEle("boxWin" + name, "div", "background-image: url(images/boxWin.png);position: absolute;left: -50px; height: 229px; top: " + topPx + "px;width: 300px;background-size: cover;z-index: 10000; transform: rotateY(180deg);", {className: "zoom-in-box"}, EDAll)), (name.length > 9 ? name.slice(0, 9) + "..." : name) + "<br/>+ " + score + " 🧠")
    ...
    if (userId != "bot") {
        ...
        if ((topLeaderBoard.length < 3) || (topLeaderBoard.length >= 3 && topLeaderBoard[2].score <= user.score)) {
            ...
            for (let i = 0; i < topLeaderBoard.length; i++) {
                if (i > 2) break;
                EditImg($("topAvatar" + (i + 1)), 0, topLeaderBoard[i].avatar);
                innerText($("topName" + (i + 1)), (topLeaderBoard[i].name.length > 11 ? topLeaderBoard[i].name.slice(0, 11) + "..." : topLeaderBoard[i].name));
                innerText($("topScore" + (i + 1)), topLeaderBoard[i].score + " 🧠");
            }
        }
    }
